...
void bcp_kv_batch_stop(void* handle, bool rollback)
{
    if (handle == NULL)
    {
        return;
...
{
    return bcp_kv_set(file, key, value, (int)strlen(value) + 1);
}

bool bcp_kv_set(const char* file, const char* key, bool value)
{
    return bcp_kv_set(file, key, value ? "1" : "0");
}
...
bool bcp_kv_set(const char* file, const char* key, int8 value)
{
    ...
    return bcp_kv_set(file, key, buf);
}
...
bool bcp_kv_set(const char* file, const char* key, double value)
{
    ...
    return bcp_kv_set(file, key, val.c_str());
}

void bcp_kv_remove_all(const char* file)
{
    bcp_file_remove(file);
}
...
{
    std::vector<KVResult> results = bcp_kv_get_front(file, count);
    for (int i = 0; i < results.size(); i++)
    {
        bcp_kv_remove(file, results[i].key.c_str());
    }
}

void bcp_kv_remove_tail(const char* file, int count)
{
    std::vector<KVResult> results = bcp_kv_get_tail(file, count);
    for (int i = 0; i < results.size(); i++)
    {
        bcp_kv_remove(file, results[i].key.c_str());
    }
...
std::string bcp_kv_get_string(const char* file, const char* key, const char* default_value)
{
    if (default_value == NULL)
    {
        default_value = "";
    }
    ByteArray bytes = bcp_kv_get_bytes(file, key);
...
bool bcp_kv_get_bool(const char* file, const char* key, bool default_value)
{
    char buf[64] = {0};
    if (bcp_kv_get_string(file, key, buf, sizeof(buf)) == false)
    {
        return default_value;
    }
...
int8 bcp_kv_get_int8(const char* file, const char* key, int8 default_value)
{
    char buf[64] = {0};
    if (bcp_kv_get_string(file, key, buf, sizeof(buf)) == false)
    {
...
uint8 bcp_kv_get_uint8(const char* file, const char* key, uint8 default_value)
{
    char buf[64] = {0};
    if (bcp_kv_get_string(file, key, buf, sizeof(buf)) == false)
    {
...
int16 bcp_kv_get_int16(const char* file, const char* key, int16 default_value)
{
    char buf[64] = {0};
    if (bcp_kv_get_string(file, key, buf, sizeof(buf)) == false)
    {
...
uint16 bcp_kv_get_uint16(const char* file, const char* key, uint16 default_value)
{
    char buf[64] = {0};
    if (bcp_kv_get_string(file, key, buf, sizeof(buf)) == false)
    {
...
int32 bcp_kv_get_int32(const char* file, const char* key, int32 default_value)
{
    char buf[64] = {0};
    if (bcp_kv_get_string(file, key, buf, sizeof(buf)) == false)
    {
...
uint32 bcp_kv_get_uint32(const char* file, const char* key, uint32 default_value)
{
    char buf[64] = {0};
    if (bcp_kv_get_string(file, key, buf, sizeof(buf)) == false)
    {
...
int64 bcp_kv_get_int64(const char* file, const char* key, int64 default_value)
{
    char buf[64] = {0};
    if (bcp_kv_get_string(file, key, buf, sizeof(buf)) == false)
    {
...
uint64 bcp_kv_get_uint64(const char* file, const char* key, uint64 default_value)
{
    char buf[64] = {0};
    if (bcp_kv_get_string(file, key, buf, sizeof(buf)) == false)
    {
