...
bool bcp_dir_create(const char* full_path)
{
    ...
    const char* delim = "/";
#endif
    std::vector<std::string> paths = bcp_string_split(full_path, delim);
    if (paths.empty())
    {
    ...
    for (int i = 0; i < paths.size(); i++)
    {
        ...
        if (bcp_file_type(path.c_str()) != FILE_TYPE_NOT_EXIST)
        {
            if (bcp_file_type(path.c_str()) != FILE_TYPE_DIR)
            {
...
int bcp_dir_remove(const char* dir_path)
{
    ...
    while ((ptr = readdir(dir)) != NULL)
    {
        ...
        if (bcp_file_type(path.c_str()) == FILE_TYPE_DIR)
        {
            bcp_dir_remove(path.c_str());
        }
        else
        {
            bcp_file_remove(path.c_str());
        }
    }
...
void bcp_file_copy(const char* file_path_to, const char* file_path_from, bool append)
{
...
{
    FILE* file = bcp_file_open(file_path, "w");
    if (file)
    {
#if defined(_WIN32) || defined(_WIN64)
...
{
    ByteArray bytes;
    FILE* file = bcp_file_open(file_path, "r");
    if (file == NULL)
    {
        return bytes;
    }
    uint8 buf[1024];
    int size = 0;
    while ((size = bcp_file_read(file, buf, sizeof(buf))) > 0)
    {
...
int bcp_file_writef(FILE* fp, const char* fmt, ...)
{
    std::string str;
    va_list args;
    va_start(args, fmt);
    ...
    return bcp_file_write(fp, str.data(), str.length());
}
...
bool bcp_file_crop(const char* file_name, uint8 start_percent_keepd, uint8 end_percent_keeped)
{
    ...
    uint64 file_size = bcp_file_size(file_name);
    if (file_size == 0)
    {
        return false;
    }
    FILE* file_in = bcp_file_open(file_name, "rb");
    if (file_in == NULL)
    {
        return false;
    }
    ...
    FILE* file_out = bcp_file_open(file_temp, "w+");
    if (file_out == NULL)
    {
        bcp_file_close(file_in);
        return false;
    }
    ...
    while (true)
    {
        size = bcp_file_read(file_in, buf, sizeof(buf));
        if (size <= 0)
        {
            break;
        }
        ...
        {
            bcp_file_write(file_out, buf, size);
        }
    ...
    free(buf);
    bcp_file_flush(file_out);
    bcp_file_close(file_out);
    bcp_file_close(file_in);
    bcp_file_remove(file_name);
    return bcp_file_rename(file_name, file_temp);
}
