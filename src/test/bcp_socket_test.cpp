...
public:
    ...
    void onRecv(uint8* data, int data_size)
    {
        ...
        ByteArray bytes(data, data_size);
...
void bcp_socket_unit_test_suit(void** state)
{
#if __linux__ == 1
    uint8 mac[LENGTH_MAC]={0};
    bcp_net_get_mac("eth0", mac);
    std::string mac_str = bcp_string_format("%02X-%02X-%02X-%02X-%02X-%02X",
                                            mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    LOG_D("mac=%s", mac_str.c_str());
    NicInfo nic;
    bcp_net_get_nic("eth0", nic);
...
void bcp_unix_domain_unit_test_suit(void** state)
{
#if __linux__ == 1
