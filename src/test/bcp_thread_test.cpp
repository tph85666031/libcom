...
static void thread_cpp_test(int val1, int val2)
{
    ...
    LOG_D("tid=%llu", bcp_thread_get_tid());
    return;
}

void bcp_thread_unit_test_suit(void** state)
{
    bcp_thread_set_name("T testthread");
    LOG_D("thread name=%s", bcp_thread_get_name(bcp_thread_get_tid_posix()).c_str());
    MyThreadPoll pool;
