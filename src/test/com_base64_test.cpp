...
{
    std::string result = ComBase64::Encode((uint8*)"1234567890", sizeof("1234567890") - 1);
    ASSERT_STR_EQUAL("MTIzNDU2Nzg5MA==", result.c_str());
    ASSERT_STR_EQUAL("1234567890", ComBase64::Decode("MTIzNDU2Nzg5MA").toString().c_str());

    result = ComBase64::Encode((uint8*)"w832y9gbvnspoVuu923_*)*)*@-", sizeof("w832y9gbvnspoVuu923_*)*)*@-") - 1);
    ASSERT_STR_EQUAL("dzgzMnk5Z2J2bnNwb1Z1dTkyM18qKSopKkAt", result.c_str());
    ASSERT_STR_EQUAL("w832y9gbvnspoVuu923_*)*)*@-", ComBase64::Decode(result.c_str()).toString().c_str());

    const char* hex = "11BA4013E73AFFD6BFC6BB01B7C7C74E61EAB8C1C49BBD3339";
    ComBytes bytes_org = com_hexstring_to_bytes(hex);
    result = ComBase64::Encode(bytes_org.getData(), bytes_org.getDataSize());
    ASSERT_STR_EQUAL("EbpAE+c6/9a/xrsBt8fHTmHquMHEm70zOQ==", result.c_str());
    ComBytes bytes = ComBase64::Decode(result.c_str());
