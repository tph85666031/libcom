...
bool Cache::expired()
{
    uint64 time = bcp_time_rtc_ms();
    if (time <= 1577808000000)//2020-01-01 00:00:00
    {
        return false;
    }
    if (flag.expiretime_ms > 0 && bcp_time_rtc_ms() > flag.expiretime_ms)
    {
        return true;
    }
    return false;
...
void CacheManager::ThreadFlushRunner(CacheManager* manager)
{
    ...
    while (manager->thread_flush_running)
    {
        ...
        bcp_sleep_ms(CACHEMANAGER_FLUSH_CHECKINTERVAL_MS);
    }
    return;
}
...
bool CacheManager::initDB()
{
    ...
    if (bcp_file_type(db_file.c_str()) != FILE_TYPE_FILE)
    {
        bcp_sqlite_close(sqlite_fd);
        sqlite_fd = NULL;
    }
    ...
    sqlite_fd = bcp_sqlite_open(db_file.c_str());
    if (sqlite_fd == NULL)
    {
        return false;
    }
    if (bcp_sqlite_is_table_exist(sqlite_fd, CACHE_MANAGER_TABLE_NAME) == false)
    {
        ...
        if (bcp_sqlite_run_sql(sqlite_fd, sql.c_str()) != BCP_SQL_ERR_OK)
        {
            bcp_sqlite_close(sqlite_fd);
            sqlite_fd = NULL;
            return false;
...
void CacheManager::insertHead(Cache& cache)
{
    ...
    {
        std::string sql = bcp_string_format("REPLACE INTO \"%s\" (%s,%s,%s,%s,%s) VALUES ('%s',%d,%lld,%d,'%s');",
                                            CACHE_MANAGER_TABLE_NAME,
                                            CACHE_MANAGER_TABLE_UUID,
                                            CACHE_MANAGER_TABLE_RETRY,
                                            CACHE_MANAGER_TABLE_EXPIRE,
        ...
        initDB();
        mutex_sqlite_fd.lock();
        if (bcp_sqlite_run_sql(sqlite_fd, sql.c_str()) == BCP_SQL_ERR_FAILED)
        {
            bcp_sqlite_close(sqlite_fd);
            sqlite_fd = NULL;
        }
...
void CacheManager::insertTail(Cache& cache)
{
    ...
    if (mem_cache_count_max == 0) //禁用内存缓存,直接写入磁盘
    {
        ...
        initDB();
        mutex_sqlite_fd.lock();
        int ret = bcp_sqlite_run_sql(sqlite_fd, sql.c_str());
...
void CacheManager::removeByUUID(const std::string& uuid)
{
    ...
    initDB();
    mutex_sqlite_fd.lock();
    if (bcp_sqlite_delete(sqlite_fd, sql.c_str()) == BCP_SQL_ERR_FAILED)
    {
        bcp_sqlite_close(sqlite_fd);
        sqlite_fd = NULL;
    }
...
void CacheManager::removeFirst(int count)
{
    initDB();
    mutex_sqlite_fd.lock();
    int count_disk = bcp_sqlite_table_row_count(sqlite_fd, CACHE_MANAGER_TABLE_NAME);
    if (count_disk > count)
    {
        count_disk = count;
    }
...
void CacheManager::removeExpired()
{
    ...
    if (time > 1577808000000)//2020-01-01 00:00:00
    {
        std::string sql = bcp_string_format("DELETE FROM \"%s\" WHERE \"%s\">0 AND \"%s\"<%lld",
                                            CACHE_MANAGER_TABLE_NAME, CACHE_MANAGER_TABLE_EXPIRE,
                                            CACHE_MANAGER_TABLE_EXPIRE, bcp_time_rtc_ms());
        initDB();
        mutex_sqlite_fd.lock();
        if (bcp_sqlite_delete(sqlite_fd, sql.c_str()) == BCP_SQL_ERR_FAILED)
        {
...
{
    std::string sql = bcp_string_format("DELETE FROM \"%s\" WHERE \"%s\"=0",
...
void CacheManager::setRetryCount(const char* uuid, int count)
{
    ...
    for (it = list.begin(); it != list.end(); it++)
    {
        if (bcp_string_equal(it->getUUID().c_str(), uuid))
        {
    ...
    std::string sql = bcp_string_format("UPDATE \"%s\" SET \"%s\"=%d WHERE \"%s\"='%s'",
                                        CACHE_MANAGER_TABLE_NAME, CACHE_MANAGER_TABLE_RETRY, count,
                                        CACHE_MANAGER_TABLE_UUID, uuid);
    initDB();
    mutex_sqlite_fd.lock();
    if (bcp_sqlite_update(sqlite_fd, sql.c_str()) == BCP_SQL_ERR_FAILED)
    {
        bcp_sqlite_close(sqlite_fd);
        sqlite_fd = NULL;
    }
...
void CacheManager::clear(bool clear_file)
{
    ...
    if (clear_file)
    {
        initDB();
        mutex_sqlite_fd.lock();
        bcp_sqlite_table_clean(sqlite_fd, CACHE_MANAGER_TABLE_NAME);
        mutex_sqlite_fd.unlock();
    }
}
...
int CacheManager::getCacheCount()
{
    ...
    initDB();
    mutex_sqlite_fd.lock();
    int val = bcp_sqlite_table_row_count(sqlite_fd, CACHE_MANAGER_TABLE_NAME);
    mutex_sqlite_fd.unlock();
    if (val > 0)
    {
        count += val;
...
void CacheManager::dilution(int step)
{
    ...
    bcp_sqlite_begin_transaction(sqlite_fd);
    for (int i = 0; i < query.getRowCount(); i = i + 1 + step)
    {
        const char* uuid = query.getItem(i, 0);
        if (uuid != NULL)
        {
            sql = bcp_string_format("DELETE FROM \"%s\" WHERE %s=\"%s\";",
                                    CACHE_MANAGER_TABLE_NAME,
                                    CACHE_MANAGER_TABLE_UUID, uuid);
    ...
    bcp_sqlite_commit_transaction(sqlite_fd);
}
