...
static uint64 my_pid = 0;
static std::atomic<void*> signal_cb_user_data;
static std::atomic<signal_cb> signal_cb_func;

static bool bcp_stack_name_of_pid(uint64 pid, char* name, int name_size)
{
...
static std::string stack_addr2line(const char* info, const char* file_addr2line, const char* file_objdump)
{
    ...
    bcp_string_trim(file);
    bcp_string_trim(function);
    bcp_string_trim(offset);
    bcp_string_trim(offset_absolute);

    uint64 addr_base = 0;
    std::string result;
    if (function.empty() == false)
    {
        result = bcp_run_shell_with_output("%s -t %s | grep %s 2>/dev/null",
                                           FILE_OBJDUMP, file.c_str(), function.c_str());
        if (result.empty())
        {
            return std::string();
        }
        std::vector<std::string> addrs = bcp_string_split(result.c_str(), " ");
        if (addrs.size() == 0)
        {
            return std::string();
        }
    ...
    result = bcp_run_shell_with_output("%s -e %s 0x%llx 2>/dev/null",
                                       FILE_ADDR2LINE, file.c_str(), addr);
    if (result[0] == '?')
    {
        return std::string();
    }
    bcp_string_replace(result, "\n", "");
...
static std::string search_file_addr2line()
{
    ...
    path = bcp_string_format("./%s", FILE_ADDR2LINE);
    if (bcp_file_type(path.c_str()) == FILE_TYPE_FILE)
    {
        return path;
    }

    path = bcp_string_format("%s/%s", bcp_get_bin_path(), FILE_ADDR2LINE);
    if (bcp_file_type(path.c_str()) == FILE_TYPE_FILE)
    {
        return path;
    }

    path = bcp_string_format("/usr/bin/%s", FILE_ADDR2LINE);
    if (bcp_file_type(path.c_str()) == FILE_TYPE_FILE)
    {
        return path;
    }
...
static std::string search_file_objdump()
{
    ...
    path = bcp_string_format("./%s", FILE_OBJDUMP);
    if (bcp_file_type(path.c_str()) == FILE_TYPE_FILE)
    {
        return path;
    }

    path = bcp_string_format("%s/%s", bcp_get_bin_path(), FILE_OBJDUMP);
    if (bcp_file_type(path.c_str()) == FILE_TYPE_FILE)
    {
        return path;
    }

    path = bcp_string_format("/usr/bin/%s", FILE_OBJDUMP);
    if (bcp_file_type(path.c_str()) == FILE_TYPE_FILE)
    {
        return path;
    }
...
{
    char pid_name[128];
    bcp_stack_name_of_pid(info->si_pid, pid_name, sizeof(pid_name));
    LOG_W("%s:%d called\nsig=%d:%s, from=%s[%u], my pid=%llu",
          __FUNC__, __LINE__, sig, strsignal(sig), pid_name, info->si_pid, my_pid);

    bcp_stack_print();

    if (signal_cb_func != NULL)
    {
        signal_cb_func(sig, signal_cb_user_data);
        return;
    }
    ...
    case SIGTTIN:
        ...
        break;
    case SIGUSR1:
        LOG_I("SIGUSR1 log config will reload");
        bcp_log_uninit();
        bcp_log_init();
        break;
    case SIGUSR2:
...
void bcp_stack_init()
{
    ...
    std::string signal_str_ignore = config.getString(group_name, "stack.signal_ignore");
    std::string signal_str_none_reset = config.getString(group_name, "stack.signal_none_reset");
    std::string signal_str_reset = config.getString(group_name, "stack.signal_reset");
    ...
    vals = bcp_string_split(signal_str_ignore.c_str(), ",");
    ...
    vals.clear();
    vals = bcp_string_split(signal_str_none_reset.c_str(), ",");
    for (int i = 0; i < vals.size(); i++)
    {
    ...
    vals.clear();
    vals = bcp_string_split(signal_str_reset.c_str(), ",");
    for (int i = 0; i < vals.size(); i++)
    {
        signal_value_reset.push_back(strtol(vals[i].c_str(), NULL, 0));
    }

    my_pid = bcp_thread_get_pid();
#ifdef __GLIBC__
...
void bcp_stack_print()
{
    ...
    std::string buf;
    buf.append("\n++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
    buf.append(bcp_string_format("  Print Stack For Thread: %s\n", bcp_thread_get_name().c_str()));
    char** callstack = backtrace_symbols(stack_addr, nstack);
    if (callstack != NULL)
    {
        for (int j = 0; j < nstack; j++)
        {
            buf.append(bcp_string_format("  ThreadID:%llu IDX:%d : %s", bcp_thread_get_tid(), j, callstack[j]));
            if (file_addr2line.empty() == false && file_objdump.empty() == false)
            {
                ...
                address_info = bcp_path_name(address_info.c_str());
...
void bcp_stack_set_hook(signal_cb cb, void* user_data)
{
    signal_cb_func = cb;
...
void bcp_system_exit(bool force)
{
#if __linux__ == 1
    bcp_system_send_signal(SIGTERM);
#else
	exit(0);
#endif
    if (force)
    {
        //信号会被程序截获，程序会执行清退工作
        bcp_sleep_s(5);//等待5秒尽可能保证程序完成清退工作
        exit(0);//强制退出
    }
}
