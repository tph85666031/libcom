...
inline bool bcp_task_exist(std::string task_name)
{
    return GetTaskManager().isTaskExist(task_name);
}

inline void bcp_task_destroy(std::string task_name)
{
    GetTaskManager().destroyTask(task_name);
}

inline void bcp_task_send_message(std::string task_name_wildcard, Message& msg)
{
    GetTaskManager().sendMessage(task_name_wildcard, msg);
}

inline void bcp_task_send_broadcast_message(Message& msg)
{
    GetTaskManager().sendBroadcastMessage(msg);
}

template<class T>
T* bcp_task_create(std::string task_name, Message init_msg = Message())
{
