...
void bcp_log_init();
void bcp_log_uninit(void);
void bcp_log_set_hook(log_hook_fc hook_fc);
void bcp_log_output(int level, const char* fmt, ...) __attribute__((format(printf, 2, 3)));

#define LOG_D(fmt,...)    bcp_log_output(LOG_LEVEL_DEBUG,   "(%s:%s:%s:%d) - " fmt, MODULE_NAME, __FILENAME__, __func__, __LINE__, ##__VA_ARGS__)
#define LOG_I(fmt,...)    bcp_log_output(LOG_LEVEL_INFO,    "(%s:%s:%s:%d) - " fmt, MODULE_NAME, __FILENAME__, __func__, __LINE__, ##__VA_ARGS__)
#define LOG_W(fmt,...)    bcp_log_output(LOG_LEVEL_WARNING, "(%s:%s:%s:%d) - " fmt, MODULE_NAME, __FILENAME__, __func__, __LINE__, ##__VA_ARGS__)
#define LOG_E(fmt,...)    bcp_log_output(LOG_LEVEL_ERROR,   "(%s:%s:%s:%d) - " fmt, MODULE_NAME, __FILENAME__, __func__, __LINE__, ##__VA_ARGS__)
#define LOG_F(fmt,...)    do{bcp_log_output(LOG_LEVEL_FATAL,"(%s:%s:%s:%d) - " fmt, MODULE_NAME, __FILENAME__, __func__, __LINE__, ##__VA_ARGS__);bcp_stack_print();}while(0)
