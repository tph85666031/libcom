...
#define bcp_thread_cleanup_push(fp, arg) pthread_cleanup_push(fp, arg)
#define bcp_thread_cleanup_pop()         pthread_cleanup_pop(0)
#else
#define bcp_thread_cleanup_push(fp, arg)
#define bcp_thread_cleanup_pop()
#endif

void bcp_thread_set_name(std::thread* t, const char* name);
void bcp_thread_set_name(std::thread& t, const char* name);
void bcp_thread_set_name(ThreadID tid_posix, const char* name);
void bcp_thread_set_name(const char* name);
std::string bcp_thread_get_name(std::thread* t);
std::string bcp_thread_get_name(std::thread& t);
std::string bcp_thread_get_name(ThreadID tid_posix);
std::string bcp_thread_get_name();
uint64 bcp_thread_get_tid_posix();
uint64 bcp_thread_get_tid();
uint64 bcp_thread_get_pid();

ThreadID bcp_thread_start(ThreadFun fp, void* arg = NULL);
void bcp_thread_detach();
bool bcp_thread_join(ThreadID tid);
bool bcp_thread_cancel(ThreadID tid);
void bcp_thread_cancel_enable();
void bcp_thread_cancel_disable();
void bcp_thread_cancel_mark();
...
public:
    ...
    void startThread()
    {
        if (thread_runner.native_handle() == bcp_thread_get_tid_posix())
        {
            return;
        }
        running = true;
...
private:
    ...
    static void RunnerWapper(EasyThread* thread)
    {
        ...
        bcp_thread_set_name(thread->name.c_str());
        while (thread->running)
        {
            uint64 time_diff = bcp_time_cpu_ms();
            thread->onLoop();
            if (bcp_time_cpu_ms() - time_diff <= 1)
            {
