 ...
 ********************************************************/
int bcp_sqlite_run_sql(void* fd, const char* sql)
{
    if (fd == NULL || sql == NULL)
    {
        LOG_W("arg incorrect");
    ...
    while (retry_count++ < 5)
    {
        ...
        else
        {
            LOG_E("failed, ret=%d, err=%s, sql=%s, file=%s, file_exist=%s",
                  ...
                  bcp_file_type(sqlite3_db_filename((sqlite3*)fd, NULL)) == FILE_TYPE_FILE ? "true" : "false");
            if (err_msg != NULL)
            {
                sqlite3_free(err_msg);
            }
...
void* bcp_sqlite_open(const char* file)
{
    ...
    sqlite3* fd = NULL;
    if (bcp_file_type(file) == FILE_TYPE_NOT_EXIST)
    {
...
const char* bcp_sqlite_file_name(void* fd)
{
    return sqlite3_db_filename((sqlite3*)fd, NULL);
}

/********************************************************
 * bcp_sqlite_query_free
 * 说明：释放数据库查询结果集
 * 入参：由bcp_sqlite_query_F生成的查询结果指针
 *
 * 返回值：无
 ********************************************************/
void bcp_sqlite_query_free(char** result)
{
    if (result)
    {
 ...
 ********************************************************/
char** bcp_sqlite_query_F(void* fd, const char* sql,
...
/********************************************************
 * bcp_sqlite_query_count
 * 说明：数据库查询并获取查询匹配的数据个数
 * 入参：fd:sqlite3句柄，sql：查询语句
 *
 * 返回值：查询匹配的数据个数
 ********************************************************/
int bcp_sqlite_query_count(void* fd, const char* sql)
{
    if (fd == NULL || sql == NULL)
    {
        return 0;
 ...
 * 返回值：成功插入的个数
 ********************************************************/
int bcp_sqlite_insert(void* fd, const char* sql)
{
    if (fd == NULL || sql == NULL)
    {
        return 0;
    }
    int ret = bcp_sqlite_run_sql(fd, sql);
 ...
 * 返回值：成功删除的个数
 ********************************************************/
int bcp_sqlite_delete(void* fd, const char* sql)
{
    if (fd == NULL || sql == NULL)
    {
        return 0;
 ...
 * 返回值：成功更新的个数
 ********************************************************/
int bcp_sqlite_update(void* fd, const char* sql)
{
    if (fd == NULL || sql == NULL)
    {
        return 0;
...
/********************************************************
 * bcp_sqlite_is_table_exist
 * 说明：查询数据表是否存在
 * 入参：fd:sqlite3句柄，table_name：表名
 *
 * 返回值：布尔值，true代表成功
 ********************************************************/
bool bcp_sqlite_is_table_exist(void* fd, const char* table_name)
{
    ...
    snprintf(sql, sizeof(sql),
             ...
             table_name);
    int count = bcp_sqlite_query_count(fd, sql);
    return count > 0  ? true : false;
}

/********************************************************
 * bcp_sqlite_table_row_count
 * 说明：查询数据表行数
 * 入参：fd:sqlite3句柄，table_name：表名
 *
 * 返回值：数据表行数
 ********************************************************/
int bcp_sqlite_table_row_count(void* fd, const char* table_name)
{
    if (fd == NULL || table_name == NULL)
    {
        return 0;
    ...
    int row_count;
    int col_count;
    char** result = bcp_sqlite_query_F(fd, sql, &row_count, &col_count);
    if (result == NULL)
    {
        LOG_E("failed");
        return 0;
...
/********************************************************
 * bcp_sqlite_table_clean
 * 说明：清空数据表并保留数据表
 * 入参：fd:sqlite3句柄，table_name：表名
 *
 * 返回值：错误码,参考BCP_SQL_ERR_XXX
 ********************************************************/
bool bcp_sqlite_table_clean(void* fd, const char* table_name)
{
    if (fd == NULL || table_name == NULL)
    {
        LOG_E("arg incorrect");
    ...
    return (bcp_sqlite_run_sql(fd, sql) == BCP_SQL_ERR_OK);
}

/********************************************************
 * bcp_sqlite_table_remove
 * 说明：删除数据库
 * 入参：fd:sqlite3句柄，table_name：表名
 *
 * 返回值：错误码,参考BCP_SQL_ERR_XXX
 ********************************************************/
bool bcp_sqlite_table_remove(void* fd, const char* table_name)
{
    if (fd == NULL || table_name == NULL)
    {
        LOG_E("arg incorrect");
    ...
    if (bcp_sqlite_table_clean(fd, table_name) == false)
    {
        LOG_E("failed");
        return false;
    }
...
/********************************************************
 * bcp_sqlite_begin_transaction
 * 说明：开启事务
 * 入参：fd:sqlite3句柄
 *
 * 返回值：布尔值，true代表成功
 ********************************************************/
bool bcp_sqlite_begin_transaction(void* fd)
{
    return (bcp_sqlite_run_sql(fd, "BEGIN TRANSACTION;") == BCP_SQL_ERR_OK);
}

/********************************************************
 * bcp_sqlite_commit_transaction
 * 说明：提交事务
 * 入参：fd:sqlite3句柄
 *
 * 返回值：布尔值，true代表成功
 ********************************************************/
bool bcp_sqlite_commit_transaction(void* fd)
{
    return (bcp_sqlite_run_sql(fd, "COMMIT TRANSACTION;") == BCP_SQL_ERR_OK);
}

/********************************************************
 * bcp_sqlite_rollback_transaction
 * 说明：回滚事务
 * 入参：fd:sqlite3句柄
 *
 * 返回值：布尔值，true代表成功
 ********************************************************/
bool bcp_sqlite_rollback_transaction(void* fd)
{
    return (bcp_sqlite_run_sql(fd, "ROLLBACK TRANSACTION;") == BCP_SQL_ERR_OK);
}
...
DBQuery::DBQuery(const void* sqlite_fd, const char* sql)
{
    if (sqlite_fd != NULL && sql != NULL)
    {
        int row_count = 0;
        int column_count = 0;
        char** data = bcp_sqlite_query_F((void*)sqlite_fd, sql, &row_count, &column_count);
        if (data == NULL)
        {
            row_count = 0;
            column_count = 0;
