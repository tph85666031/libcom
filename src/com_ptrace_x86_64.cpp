...
PTrace_x86_64::PTrace_x86_64(ProcessID pid) : PTrace(pid)
{
...
void* PTrace_x86_64::getEntryAddrLib(ProcessID pid, const char* file_lib)
{
    std::string file_map = "/proc/self/maps";
    if(pid > 0)
    {
...
void* PTrace_x86_64::getSymbolAddr(const char* sym_name)
{
    ...
    do
    {
        ...
        do
        {
            ...
            {
                addr_symtab = (Elf64_Sym*)dyn.d_un.d_val;
            }
...
void* PTrace_x86_64::getGotAddr(const char* sym_name)
{
    ...
    for(int i = 0; i < rel_count; i++)
    {
        ...
        pread((void*)((Elf64_Addr)addr_symtab + ELF64_R_SYM(rel.r_info)*sizeof(Elf64_Sym)), &sym, sizeof(sym));
        std::string name = preadString(addr_strtab + sym.st_name);
        if(com_string_equal(name.c_str(), sym_name))
        {
            //获取指向got对应位置的指针
            return (void*)((Elf64_Addr)addr_entry + rel.r_offset);
        }
    }
...
bool PTrace_x86_64::loadElf()
{
    ...
    Elf64_Addr* phdr_addr = (Elf64_Addr*)((unsigned long)addr_entry + ehdr.e_phoff);
    // 遍历program headers table，找到.dynamic
    LOG_D("count=%d,%s,0x%zx", ehdr.e_phnum, ehdr.e_ident, ehdr.e_entry);
    for(int i = 0; i < ehdr.e_phnum; i++)
    {
        pread((unsigned char*)phdr_addr + i * sizeof(Elf64_Phdr), &phdr, sizeof(phdr));
        assert(phdr.p_align == 0 || (phdr.p_vaddr % phdr.p_align) == (phdr.p_offset % phdr.p_align));
        if(phdr.p_type == PT_DYNAMIC)
        {
            addr_dynamic = (void*)((Elf64_Addr)addr_entry + phdr.p_vaddr);
            break;
        }
    }
    ...
    do
    {
        pread((void*)((Elf64_Addr)addr_dynamic + index * sizeof(Elf64_Dyn)), &dyn, sizeof(Elf64_Dyn));
        ...
        {
            addr_rel = (Elf64_Rela*)dyn.d_un.d_val;
            LOG_D("addr_rel=%p", addr_rel);
        }
        else if(dyn.d_tag == DT_JMPREL)
        {
            addr_plt_rel = (Elf64_Rela*)dyn.d_un.d_val;
    ...
    pread((void*)((Elf64_Addr)addr_gotplt + sizeof(Elf64_Addr)), &addr_link_map, sizeof(void*));
    LOG_D("link_map=%p", addr_link_map);
    if(addr_link_map == 0)
    {
        pread((void*)((Elf64_Addr)addr_debug + sizeof(Elf64_Addr)), &addr_link_map, sizeof(void*));
        LOG_D("link_map=%p", addr_link_map);
    }
    return true;
}
...
bool PTrace_x86_64::replace(const char* func_name, const char* func_name_new)
{
    ...
    Elf64_Addr addr_new = (Elf64_Addr)getSymbolAddr(func_name_new);

    return pwrite(addr_plt, &addr_new, sizeof(addr_new));
}

bool PTrace_x86_64::replace(const char* func_name, Elf64_Addr addr_new)
{
