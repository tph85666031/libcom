...
#define DNS_PORT 53
static std::atomic<uint16_t> dns_id = {0};
...
CPPBytes dns_query_encode(const char* domain_name, uint16_t id, bool is_tcp)
{
    //构造DNS数据
    Serializer s;
    s.append((uint16_t)id);
    s.append((uint16_t)0x0100);
    s.append((uint16_t)1);
    s.append((uint16_t)0);
    s.append((uint16_t)0);
    s.append((uint16_t)0);
    ...
    {
        s.append((uint8_t)name_parts[i].size());
        s.append(name_parts[i]);
    }
    s.append((uint8_t)0);
    s.append((uint8_t)0x00);
    s.append((uint8_t)0x01);
    s.append((uint8_t)0x00);
    s.append((uint8_t)0x01);
    if(is_tcp == false)
    {
    ...
    Serializer s2;
    s2.append((uint16_t)s.getDataSize());
    s2.append(s.getData(), s.getDataSize());
    return s2.toBytes();
}

std::string dns_query_decode(uint8_t* buf, int buf_size, uint16_t id_expect, bool is_tcp)
{
    ...
    for(int i = 0; i < answer_count; i++)
    {
        ...
        uint16_t answer_class = 0;
        uint32_t answer_ttl = 0;
...
std::string com_dns_query(const char* domain_name, const char* interface_name, const char* dns_server_ip)
{
    ...
    if(com_string_len(interface_name) <= 0)
    {
        ...
        uint32_t ip = ntohl(((sockaddr_in*)(res->ai_addr))->sin_addr.s_addr);
        freeaddrinfo(res);
    ...
    bool rp_set = false;
    uint8_t rpfilter_flag = com_net_get_rpfilter(interface_name);
    if(rpfilter_flag != 2)
    {
