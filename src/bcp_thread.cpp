...
ThreadID bcp_thread_start(ThreadFun fp, void* arg)
{
#if defined(_WIN32) || defined(_WIN64)
    return NULL;
#else
...
void bcp_thread_detach()//调用后无需使用join等待线程退出
{
#if __linux__ == 1
...
void bcp_thread_set_name(const char* name)
{
    ...
    info.dwType = 0x1000;
    info.szName = name;
    info.dwThreadID = bcp_thread_get_tid();
    info.dwFlags = 0;
    __try
    {
...
void ThreadPool::createThread(int count)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        bcp_thread_set_name(t, bcp_string_format("T-Poll %llu", bcp_thread_get_tid()).c_str());
