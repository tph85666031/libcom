...
std::string bcp_com_search_config_file()
{
    std::vector<std::string> config_files;

    const char* bin_path = bcp_get_bin_path();
    const char* bin_name = bcp_get_bin_name();

    config_files.push_back(bcp_string_format("./config/%s_com.ini", bin_name));
    config_files.push_back(bcp_string_format("./config/%s", FILE_COM_CONFIG));
    config_files.push_back(bcp_string_format("./%s_com.ini", bin_name));
    config_files.push_back(bcp_string_format("./%s", FILE_COM_CONFIG));
    config_files.push_back(bcp_string_format("%s/config/%s_com.ini", bin_path, bin_name));
    config_files.push_back(bcp_string_format("%s/config/%s", bin_path, FILE_COM_CONFIG));
    config_files.push_back(bcp_string_format("%s/%s_com.ini", bin_path, bin_name));
    config_files.push_back(bcp_string_format("%s/%s", bin_path, FILE_COM_CONFIG));
    config_files.push_back(bcp_string_format("./tmp/%s/depend/%s", HOST_TYPE, FILE_COM_CONFIG));

    for (int i = 0; i < config_files.size(); i++)
    {
        if (bcp_file_type(config_files[i].c_str()) == FILE_TYPE_FILE)
        {
...
bool bcp_run_shell(const char* fmt, ...)
{
#if __linux__ == 1
    std::string cmd;
    va_list args;
...
std::string bcp_run_shell_with_output(const char* fmt, ...)
{
    std::string result;
#if __linux__ == 1
    if (fmt == NULL)
...
bool bcp_strncmp_ignore_case(const char* str1, const char* str2, int size)
{
...
bool bcp_strncmp(const char* str1, const char* str2, int size)
{
    if (str1 == NULL || str2 == NULL || size <= 0)
    {
        return false;
...
bool bcp_string_equal(const char* a, const char* b)
{
    if (a == NULL || b == NULL)
    {
        return false;
...
{
    return bcp_string_trim_left(bcp_string_trim_right(str, t), t);
}

char* bcp_string_trim_left(char* str)
{
    if (str == NULL)
    {
        return NULL;
...
char* bcp_string_trim_right(char* str)
{
    if (str == NULL)
    {
        return NULL;
...
char* bcp_string_trim(char* str)
{
    bcp_string_trim_right(str);
    bcp_string_trim_left(str);
    return str;
}

bool bcp_string_start_with(const char* str, const char* prefix)
{
...
bool bcp_string_end_with(const char* str, const char* tail)
{
    if (str == NULL || tail == NULL)
    {
        return false;
    }
    int l1 = bcp_string_len(str);
    int l2 = bcp_string_len(tail);
    if (l1 >= l2)
    {
        if (bcp_string_equal(str + l1 - l2, tail))
        {
            return true;
        }
    }
...
bool bcp_string_contain(const char* str, const char* sub)
{
...
std::string& bcp_string_to_upper(std::string& str)
{
    std::transform(str.begin(), str.end(), str.begin(), ::toupper);
    return str;
}

std::string& bcp_string_to_lower(std::string& str)
{
    std::transform(str.begin(), str.end(), str.begin(), ::tolower);
    return str;
}

char* bcp_string_to_upper(char* str)
{
    if (str == NULL)
    {
        return str;
...
char* bcp_string_to_lower(char* str)
{
    if (str == NULL)
    {
        return str;
...
std::string bcp_string_format(const char* fmt, ...)
{
    std::string str;
    va_list args;
    va_start(args, fmt);
...
/* 匹配 * ？通配符 */
bool bcp_string_match(const char* str, const char* pattern)
{
#if __linux__ == 0
    return bcp_string_equal(str, pattern);
#else
    return (fnmatch(pattern, str, 0) == 0);
#endif
}

std::string bcp_bytes_to_hexstring(const uint8* data, uint16 size)
{
    char buf[8];
    std::string result;
    if (data != NULL && size > 0)
...
ByteArray bcp_hexstring_to_bytes(const char* str)
{
    ByteArray bytes;
    if (str == NULL)
    {
...
int bcp_hexstring_to_bytes(const char* str, unsigned char* bytes, int size)
{
    char* strEnd;
    int m = 0;
...
bool bcp_string_replace(char* str, char from, char to)
{
    int len = bcp_string_len(str);
    if (len <= 0)
    {
        return false;
    }
...
bool bcp_string_replace(std::string& str, const char* from, const char* to)
{
    ...
    int pos = 0;
    int from_len = bcp_string_len(from);
    int to_len = bcp_string_len(to);
...
int bcp_string_len(const char* str)
{
    if (str == NULL)
    {
        return 0;
...
int bcp_string_size(const char* str)
{
    if (str == NULL)
    {
        return 0;
    }
    return bcp_string_len(str) + 1;
}

bool bcp_string_is_ip(const char* ip)
{
    if (ip == NULL)
    {
        return false;
...
int bcp_snprintf(char* buf, int buf_size, const char* fmt, ...)
{
...
void bcp_sleep_s(uint32 val)
{
    bcp_sleep_ms((uint64)val * 1000);
}

uint32 bcp_rand(uint32 min, uint32 max)
{
    static uint32 seed = 0;
    srand(bcp_time_cpu_ms() + (seed++));
    return (uint32)(rand() % ((uint64)max - min + 1) + min);
}

bool bcp_gettimeofday(struct timeval* tp)
{
    if (tp == NULL)
    {
        return false;
...
uint64 bcp_time_diff_ms(uint64 start, uint64 end)
{
    if (end < start)
    {
...
%% 百分号
*/
std::string bcp_time_to_string(uint32 time_s, const char* format)
{
    std::string str;
    if (format == NULL)
    {
    ...
    if (bcp_time_to_tm(time_s, &tm_val) == false)
    {
        return str;
    }
    char buf[128];
...
std::string bcp_time_to_string(uint32 time_s)
{
    return bcp_time_to_string(time_s, "%Y-%m-%d %H:%M:%S");
}

//UTC
bool bcp_time_to_tm(uint32 time_s,  struct tm* tm_val)
{
    if (tm_val == NULL)
    {
        return false;
...
//UTC
bool bcp_time_to_bcd(uint32 time_s, uint8 bcd_time[6])
{
    if (bcd_time == NULL)
    {
        return false;
    ...
    return bcp_tm_to_bcd_date(&tm_val, bcd_time);
}

uint32 bcp_time_from_bcd(uint8 bcd_date[6])
{
    if (bcd_date == NULL)
    {
        return 0;
    ...
    struct tm tm_val;
    memset(&tm_val, 0, sizeof(struct tm));
    tm_val.tm_year = 2000 + bcp_bcd_to_uint8(bcd_date[0]) - 1900;
    tm_val.tm_mon = bcp_bcd_to_uint8(bcd_date[1]) - 1;
    tm_val.tm_mday = bcp_bcd_to_uint8(bcd_date[2]);

    tm_val.tm_hour = bcp_bcd_to_uint8(bcd_date[3]);
    tm_val.tm_min = bcp_bcd_to_uint8(bcd_date[4]);
    tm_val.tm_sec = bcp_bcd_to_uint8(bcd_date[5]);

    return bcp_tm_to_time(&tm_val);
}

//UTC
uint32 bcp_tm_to_time(struct tm* tm_val)
{
    if (tm_val == NULL)
    {
        return 0;
    ...
    return (uint32)(mktime(tm_val) + bcp_timezone_get_s());
}

//UTC
bool bcp_tm_to_bcd_date(struct tm* tm_val, uint8 bcd_date[6])
{
    ...
    {
        bcd_date[0] = bcp_uint8_to_bcd(tm_val->tm_year - 100);
    }
    else
    {
        bcd_date[0] = 100 + tm_val->tm_year;
    }
    bcd_date[1] = bcp_uint8_to_bcd(tm_val->tm_mon + 1);
    bcd_date[2] = bcp_uint8_to_bcd(tm_val->tm_mday);
    bcd_date[3] = bcp_uint8_to_bcd(tm_val->tm_hour);
    bcd_date[4] = bcp_uint8_to_bcd(tm_val->tm_min);
    bcd_date[5] = bcp_uint8_to_bcd(tm_val->tm_sec);
    return true;
}

/* UTC date_str格式为 yyyy-mm-dd HH:MM:SS */
uint32 bcp_string_to_time(const char* date_str)
{
    if (date_str == NULL)
    {
        return 0;
...
uint8 bcp_uint8_to_bcd(uint8 v)
{
    return ((v / 10) << 4 | v % 10);
}

uint8 bcp_bcd_to_uint8(uint8 bcd)
{
    uint8 tmp = 0;
    tmp = ((bcd & 0xF0) >> 0x4) * 10;
    return (tmp + (bcd & 0x0F));
...
bool bcp_sem_init(Sem* sem, const char* name)
{
    if (sem == NULL)
    {
        return false;
...
Sem* bcp_sem_create(const char* name)
{
    if (name == NULL)
    {
        name = "Unknown";
...
bool bcp_sem_wait(Sem* sem, int timeout_ms)
{
    if (sem == NULL)
    {
        LOG_E("arg incorrect");
...
bool bcp_condition_init(Condition* condition, const char* name)
{
    if (condition == NULL)
    {
        return false;
...
bool bcp_condition_uninit(Condition* condition)
{
    if (condition == NULL)
    {
        LOG_E("%s:%d failed", __FUNC__, __LINE__);
...
Condition* bcp_condition_create(const char* name)
{
    if (name == NULL)
    {
        name = "Unknown";
...
bool bcp_condition_wait(Condition* condition, Mutex* mutex, int timeout_ms)
{
...
bool bcp_condition_notify_one(Condition* condition)
{
    if (condition == NULL)
    {
...
bool bcp_condition_notify_all(Condition* condition)
{
    if (condition == NULL)
    {
        LOG_E("%s:%d arg incorrect", __FUNC__, __LINE__);
...
bool bcp_condition_destroy(Condition* condition)
{
    if (condition == NULL)
    {
        LOG_E("%s:%d failed", __FUNC__, __LINE__);
...
bool bcp_mutex_init(Mutex* mutex, const char* name)
{
    if (mutex == NULL)
    {
        return false;
...
bool bcp_mutex_uninit(Mutex* mutex)
{
    if (mutex == NULL)
    {
        LOG_E("%s:%d failed", __FUNC__, __LINE__);
...
Mutex* bcp_mutex_create(const char* name)
{
    if (name == NULL)
    {
        name = "Unknown";
...
bool bcp_mutex_destroy(Mutex* mutex)
{
    if (mutex == NULL)
    {
        LOG_E("%s:%d failed", __FUNC__, __LINE__);
...
bool bcp_mutex_lock(Mutex* mutex)
{
    if (mutex == NULL)
    {
        LOG_E("%s:%d failed", __FUNC__, __LINE__);
    ...
    LOG_D("mutex=%s thread=%s LOCKED", mutex->name, bcp_thread_get_name().c_str());
#endif
#if defined(_WIN32) || defined(_WIN64)
    if (mutex->handle == NULL)
    {
...
bool bcp_mutex_unlock(Mutex* mutex)
{
    if (mutex == NULL)
    {
        LOG_E("%s:%d failed", __FUNC__, __LINE__);
    ...
    LOG_D("mutex=%s thread=%s UNLOCKED",
          mutex->name, bcp_thread_get_name().c_str());
#endif
    return true;
}

double bcp_cycle_perimeter(double diameter)
{
    return diameter * BCP_PI / 180.0;
}

double bcp_gps_distance_m(double lon_a, double lat_a, double lon_b,
                          double lat_b)
{
    double radLat1 = bcp_cycle_perimeter(lat_a);
    double radLat2 = bcp_cycle_perimeter(lat_b);
    double a = radLat1 - radLat2;
    double b = bcp_cycle_perimeter(lon_a) - bcp_cycle_perimeter(lon_b);
...
static bool bcp_gps_is_outsize_china(double longitude, double latitude)
{
...
GPS bcp_gps_wgs84_to_gcj02(double longitude, double latitude)
{
    GPS gps;
    memset(&gps, 0, sizeof(GPS));
    if (bcp_gps_is_outsize_china(longitude, latitude))
    {
...
GPS bcp_gps_gcj02_to_wgs84(double longitude, double latitude)
{
    GPS gps;
    GPS gps_temp;
    GPS gps_shift;
    ...
    while (true)
    {
        gps_temp = bcp_gps_wgs84_to_gcj02(gps.longitude, gps.latitude);
...
GPS bcp_gps_bd09_to_gcj02(double longitude, double latitude)
{
...
GPS bcp_gps_gcj02_to_bd09(double longitude, double latitude)
{
    double x = longitude;
    double y = latitude;
    double z = sqrt(x * x + y * y) + 0.00002 * sin(y * BCP_PI_GPS_BD);
...
//ip是大端结构
uint32 bcp_string_to_ip(const char* ip_str)
{
    uint32 ip[4];
...
std::string bcp_ip_to_string(uint32 ip)
{
    static char str[64];
...
void bcp_set_cwd(const char* dir)
{
    if (dir != NULL)
    {
#ifdef _WIN32
...
uint64 bcp_ptr_to_number(const void* ptr)
{
    uint64 val = 0;
...
void* bcp_number_to_ptr(const uint64 val)
{
    void* ptr = NULL;
...
std::string bcp_uuid_generator()
{
    std::string val = bcp_string_format("%llu-%llu-%llu-%llu-%u",
                                        bcp_thread_get_pid(),
                                        bcp_thread_get_tid(),
                                        bcp_time_rtc_us(),
                                        bcp_time_cpu_us(),
                                        bcp_rand(0, 0xFFFFFFFF));
    CPPMD5 md5;
...
//计算最大公约数
int bcp_gcd(int x, int y)
{
    return y ? bcp_gcd(y, x % y) : x;
}
...
{
    std::string str = bcp_bytes_to_hexstring(getData(), getDataSize());
    if (upper)
    {
        bcp_string_to_upper(str);
    }
    return str;
}
...
{
    return bcp_hexstring_to_bytes(hex_str);
}
...
{
#ifdef __DEBUG_MUTEX__
    LOG_D("mutex=%s thread=%s LOCKED", name.c_str(), bcp_thread_get_name().c_str());
#endif
    mutex.lock();
}
...
void CPPMutex::unlock()
{
    mutex.unlock();
#ifdef __DEBUG_MUTEX__
    LOG_D("mutex=%s thread=%s UNLOCKED", name.c_str(), bcp_thread_get_name().c_str());
#endif
}
...
bool CPPSem::post()
{
    return bcp_sem_post(&sem);
}

bool CPPSem::wait(int timeout_ms)
{
    return bcp_sem_wait(&sem, timeout_ms);
}
