...
std::vector<std::string> dns_get_server_list(const char* file_dns)
{
    ...
    FILE* file = bcp_file_open(file_dns, "r");
    if (file != NULL)
    {
        char line[1024];
        while (bcp_file_readline(file, line, sizeof(line)))
        {
            if (!bcp_string_start_with(line, "nameserver"))
            {
                continue;
            }
            std::vector<std::string> vals = bcp_string_split(line, " ");
            if (vals.size() != 2)
            {
                continue;
            }
            if (bcp_string_is_ip(vals[1].c_str()))
            {
...
ByteArray dns_query_encode(const char* domain_name, uint16 id, bool is_tcp)
{
    //构造DNS数据
    Serializer s;
    s.append((uint16)id);
    ...
    s.append((uint16)0);
    s.append((uint16)0);
    std::vector<std::string> name_parts = bcp_string_split(domain_name, ".");
...
std::string dns_query_decode(uint8* buf, int buf_size, uint16 id_expect, bool is_tcp)
{
    ...
    for (int i = 0; i < answer_count; i++)
    {
        ...
        {
            return bcp_string_format("%u.%u.%u.%u", bytes_tmp[0], bytes_tmp[1], bytes_tmp[2], bytes_tmp[3]);
        }
    }
...
{
    int socket_fd = bcp_socket_udp_open(interface, 10053, false);
    if (socket_fd <= 0)
    {
    ...
    int ret = bcp_socket_udp_send(socket_fd, dns_server_ip, DNS_PORT, bytes.getData(), bytes.getDataSize());
    if (ret <= 0)
    {
        bcp_socket_close(socket_fd);
        return std::string();
    }

    uint8 buf[1024];
    ret = bcp_socket_udp_read(socket_fd, buf, sizeof(buf), 5 * 1000, NULL, 0);
    if (ret <= 0)
    {
        bcp_socket_close(socket_fd);
        return std::string();
...
{
    LOG_E("called");
    int socket_fd = bcp_socket_tcp_open(dns_server_ip, DNS_PORT, 5 * 1000, interface);
    if (socket_fd <= 0)
    {
    ...
    int ret = bcp_socket_tcp_send(socket_fd, bytes.getData(), bytes.getDataSize());
    if (ret <= 0)
    {
        bcp_socket_close(socket_fd);
        return std::string();
    ...
    uint8 buf[1024];
    ret = bcp_socket_tcp_read(socket_fd, buf, sizeof(buf), 5 * 1000, false);
    if (ret <= 0)
    {
        bcp_socket_close(socket_fd);
        return std::string();
...
std::string bcp_dns_query(const char* domain_name, const char* interface_name, const char* dns_server_ip)
{
    if (domain_name == NULL)
    {
    ...
    if (bcp_string_len(interface_name) <= 0)
    {
        addrinfo hints;
        addrinfo* res = NULL;
        ...
        freeaddrinfo(res);
        return bcp_string_format("%u.%u.%u.%u", (ip >> 24) & 0xFF, (ip >> 16) & 0xFF,
                                 (ip >> 8) & 0xFF, ip & 0xFF);
    }
    ...
    else if (bcp_string_is_ip(dns_server_ip))
    {
    ...
    bool rp_set = false;
    uint8 rpfilter_flag = bcp_net_get_rpfilter(interface_name);
    if (rpfilter_flag != 2)
    {
        bcp_net_set_rpfilter(interface_name, 2);
        rp_set = true;
    }
    std::string ip;
    ...
    if (rp_set)
    {
        bcp_net_set_rpfilter(interface_name, rpfilter_flag);
    }
