...
bool bcp_mem_get_bool(const char* key, bool default_val)
{
    AutoMutex mutex(&mutex_share_mem);
...
double bcp_mem_get_double(const char* key, double default_val)
{
    AutoMutex mutex(&mutex_share_mem);
    return mem_msg.getDouble(key, default_val);
}

int8 bcp_mem_get_int8(const char* key, int8 default_val)
{
    AutoMutex mutex(&mutex_share_mem);
    return mem_msg.getInt8(key, default_val);
}

int16 bcp_mem_get_int16(const char* key, int16 default_val)
{
    AutoMutex mutex(&mutex_share_mem);
    return mem_msg.getInt16(key, default_val);
}

int32 bcp_mem_get_int32(const char* key, int32 default_val)
{
    AutoMutex mutex(&mutex_share_mem);
    return mem_msg.getInt32(key, default_val);
}

int64 bcp_mem_get_int64(const char* key, int64 default_val)
{
    AutoMutex mutex(&mutex_share_mem);
    return mem_msg.getInt64(key, default_val);
}

uint8 bcp_mem_get_uint8(const char* key, uint8 default_val)
{
    AutoMutex mutex(&mutex_share_mem);
    return mem_msg.getUInt8(key, default_val);
}

uint16 bcp_mem_get_uint16(const char* key, uint16 default_val)
{
    AutoMutex mutex(&mutex_share_mem);
    return mem_msg.getUInt16(key, default_val);
}

uint32 bcp_mem_get_uint32(const char* key, uint32 default_val)
{
    AutoMutex mutex(&mutex_share_mem);
    return mem_msg.getUInt32(key, default_val);
}

uint64 bcp_mem_get_uint64(const char* key, uint64 default_val)
{
    AutoMutex mutex(&mutex_share_mem);
...
{
    FILE* file_fd = bcp_file_open(file.c_str(), "w+");
    if (file_fd == NULL)
    {
        return false;
    }
    std::string json = bcp_mem_to_json();
    int ret = bcp_file_write(file_fd, json.data(), json.size());
    bcp_file_close(file_fd);
    return (ret == json.size());
}

void bcp_mem_from_file(std::string file)
{
    ByteArray bytes = bcp_file_readall(file.c_str());
    bcp_mem_from_json(bytes.toString());
}
