...
{
    if (com_string_len(file_name) > UD_IPC_NAME_LEN)
    {
...
void UnixDomainIPCServer::forwardMessage(std::string& from_file_name, uint8* data, int data_size)
{
    ...
    while (des.bytes.getDataSize() >= (int)sizeof(UD_IPC_MSG))
    {
        ...
        if (com_string_equal(getServerFileName().c_str(), head->to))
        {
            std::string from;
...
{
    if (com_string_len(server_file_name) > UD_IPC_NAME_LEN || com_string_len(file_name) > UD_IPC_NAME_LEN)
    {
...
bool UnixDomainIPCClient::sendMessage(const char* to_file_name, uint8* data, int data_size)
{
    ...
    msg.sof = UD_IPC_SOF;
    com_strncpy(msg.from, getFileName().c_str(), sizeof(msg.from));
...
void UnixDomainIPCClient::onRecv(uint8* data, int data_size)
{
    ...
    while (bytes.getDataSize() >= (int)sizeof(UD_IPC_MSG))
    {
        ...
        if (com_string_match(getFileName().c_str(), head->to))
        {
            std::string from;
            from.assign(head->from, sizeof(head->from));
